...
const logger = LoggerFactory.getLogger('service-worker');
...
if(DEBUG_MODE) {
    logger.debug(`Service worker version ${SERVICE_WORKER_VERSION} loading...`)
}
...
if(DEBUG_MODE) {
    logger.debug(`${componentName}:: Assets that will be cached: `, assetsToCache)
}
...
// where the browser does not know how to handle deep links
...
self.addEventListener('message', (event: { data: any; type: any; ports: any }) => {
    ...
    if(event && event.data && event.data.type) {
        ...
        if('GET_VERSION' === event.data.type) {
            if(DEBUG_MODE) {
                logger.debug(`${componentName}:: Returning the service worker version: ${SERVICE_WORKER_VERSION}`)
            }
        ...
        if('SKIP_WAITING' === event.data.type) {
            if(DEBUG_MODE) {
                logger.debug(`${componentName}:: Skipping waiting...`)
            }
            self.skipWaiting()
        }
        ...
        if('CLIENTS_CLAIM' === event.data.type) {
            if(DEBUG_MODE) {
                logger.debug(`${componentName}:: Claiming clients and cleaning old caches`)
            }
            self.clients.claim()
        }
    }
